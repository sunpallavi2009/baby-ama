    ...
    <section class="doctor-patinet-appointment pt-5">
        ...
        <div class="doc-pat-upload-report">
            ...
            <div class="row py-3">
                <div class="col-12">
                    <form
                        action="{{ route('doctor.appointment.patient.reports.post', ['appoinment' => $appoinment->id, 'patient' => $user->patient->id]) }}"
                        ...
                        <div class="row">
                            ...
                            <div class="col form-fld">
                                {{-- <input type="text"
                                    class="form-control @error('report_type') {{ 'error' }} @enderror"
                                    placeholder="Report Type" id="insReportType" name="report_type"
                                    value="{{ old('report_type') }}"> --}}

                                <select id="report_type" name="report_type" class="form-select" aria-label="select type"
                                    required>
            ...
            <div class="reports-gallery py-4">
                                ...
                                <div class="btn-group float-end">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li><a class="dropdown-item" href="#"
                                                onclick="reportlist('name','asc','<?php echo $patientid; ?>')">Name A to Z</a>
                                        </li>
                                        <li><a class="dropdown-item" href="#"
                                                onclick="reportlist('name','desc','<?php echo $patientid; ?>')">Name Z to A</a>
                                        </li>
                                        <li><a class="dropdown-item" href="#"
                                                onclick="reportlist('date','asc','<?php echo $patientid; ?>')">Date Oldest to
                                                Newest</a></li>
                                        <li><a class="dropdown-item" href="#"
                                                onclick="reportlist('date','desc','<?php echo $patientid; ?>')">Date Newest to
                                                Oldest</a></li>
                                    </ul>
                                </div>
                            </div>
                ...
                <div class="row g-3 my-3 reps">
                    ...
                    @foreach ($get_reports as $key => $getreports)
                        <!-- single report -->
                        <div id="report_{{ $getreports->id }}" class="col-6 col-md-4 col-lg-3 p-3">
                            <div class="report">
                                ...
                                <div class="report-info p-3">
                                    <div class="info">
                                        ...
                                        <p class="r-name mb-1">{{ $getreports->report_type }}</p>
                                        <p class="text-muted r-date mb-0">
                                            {{ helperParseDate($getreports->report_date, 'Y-m-d', 'd M Y') }}
                                        </p>
                                    </div>
                                    <div class="actions">
                                        <div class="dropdown dropstart"> {{-- dropup dropup-center  --}}
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                <li><button class="dropdown-item" type="button" data-bs-toggle="modal"
                                                        data-bs-target="#reporEdit" data-id="{{ $getreports->id }}"
                                                        onclick="editFunc(<?php echo $getreports->id; ?>)">Edit</button></li>
                                                <li><button class="dropdown-item report_class" type="button"
                                                        data-bs-toggle="modal" data-bs-target="#reporDelete"
                                                        data-id="{{ $getreports->id }}"
                                                        onclick="delFunc({{ $getreports->id }})">Delete</button></li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="modal fade" id="reporEdit" tabindex="-1" aria-labelledby="reporEditLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <form
                    action="{{ route('doctor.patient.update.report.post', ['appoinment' => $appoinment->id, 'patient' => $user->patient->id]) }}"
                    method="POST" enctype="multipart/form-data">
                    @csrf
                        <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="reporEditLabel">Update Report </h1>
                                <button type="reset" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                            </div>
                            <div class="modal-body row justify-content-between gy-4 mx-0 reportEdit">
                                <div class="col-12 mb-3">
                                    <label for="reportName">Report Name</label>
                                    <input type="text" class="form-control mt-2" id="reportName" name="reportName"
                                        placeholder="Update Report Name">
                                        <input type="hidden" name="reportId">
                                </div>
                                <div class="col-12 col-md-6 form-fl">
                                    <label for="report_type">Report Type</label>
                                    <select id="report_type" name="reportType" class="mt-2 form-select"
                                ...
                                <div class="col-12 col-md-6">
                                    ...
                                    <input type="date" class="form-control mt-2" id="reportDate" name="reportDate">
                                </div>
                                <div class="col-12 form-fld">
                                    <label for="insFilereportUpload">Select File</label>
                                    <div class="mb-3 selec-file mt-2 w-100">
                                        <label for="insFilereportUpload" class="form-label btn-select-file">select
                                            File</label>
                                        <input class="form-control" type="file" name="reportImage"
                                            id="insFilereportUpload" accept="image/png, image/jpeg"
                            ...
                            <div class="modal-footer">
                                <button type="reset" class="baby-secondary-btn border-1 text-center"
            ...
            {{-- Delete Report --}}
            <div class="modal fade" id="reporDelete" tabindex="-1" aria-labelledby="reporDeleteLabel"
                aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-danger" data-bs-dismiss="modal"
                                onclick="deleteReport();">Delete</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
        ...
        jQuery.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#report_' + delid).remove();
                } else {
                    toastr.error(response.message);
                }
            },
            error: function() {
                toastr.warning('Warning: Error deleting record');
                console.log(xhr.responseText);
            }
        });
    }

    function delFunc(report_id) {
        $("#del_report").val(report_id);
    }

    function editFunc(report_id) {
    var p_id = report_id;
    var actionUrl = "{{ route('doctor.patient.edit.report', ':p_id') }}";
    // Replace placeholder with actual report_id in actionUrl
    actionUrl = actionUrl.replace(':p_id', p_id);
    ...
    headers: {
        ...
        success: function(response) {
            ...
            var reportEditForm = $('#reporEdit form');
            reportEditForm.find('input[name="reportName"]').val(res.report_name);
            reportEditForm.find('input[name="reportId"]').val(res.id);
            reportEditForm.find('select[name="reportType"]').val(res.report_type);
            reportEditForm.find('input[name="reportDate"]').val(res.report_date);
        },
